...
export const searchForProducts = createTool({
  description: `Use this tool when the user is asking about electrical products or equipment. 
    ...
    The user might ask for a specific product by its product number as well.`,
  parameters: z.object({
    question: z
      .string()
      .describe(
    ...
    productNumber: z
      .string()
      .describe(
        "The product number of the product the user is looking for. Keep empty if the user is not asking for a specific product by its product number."
      ),
  }),
  execute: async ({ question, productNumber }) => {
    console.log(`Product search query: ${question}`);
    const products = await findRelevantProduct(question, productNumber);
    return products;
  },
});
