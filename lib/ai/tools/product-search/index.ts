...
export const searchForProducts = createTool({
  ...
  execute: async ({ question, productNumbers }) => {
    ...
    if (productNumbers) {
      ...
      if (numbers.length > 1) {
        ...
        const productPromises = numbers.map((num) =>
          findRelevantProduct(question, num)
        );
        ...
        return findRelevantProduct(question, productNumbers);
      }
    } else {
      // No product number specified
      return findRelevantProduct(question);
    }
  },
});
